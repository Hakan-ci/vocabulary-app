...
export type AIPracticeProps={persistence:PracticePersistence;quiz:TestSession;history:LearningHistory;catalog:readonly VocabularyWord[];onExit:()=>void;providerFactory?:()=>AIPracticeProvider;loadTransport?:typeof approvedPracticeTransport}
export function AIPractice(props:AIPracticeProps){
  ...
  return <section className="test-panel ai-practice" aria-label="AI practice setup">
    ...
    <p className="storage-notice">{mode==='voiceAnswer'?'Typed translation checked locally — no model calls':provider==='openai'?'OpenAI text tutoring — selected vocabulary and finalized messages are sent to OpenAI':'Mock practice — no AI service connected'}</p>{transport&&mode!=='voiceAnswer'&&<label className="ai-mode">Tutor<select value={provider} onChange={event=>setProvider(event.target.value)}><option value="mock">Mock tutor</option><option value="openai">OpenAI tutor</option></select></label>}{!transport&&<p>Real AI is available only to approved signed-in accounts when the pilot is enabled.</p>}<p>Your quiz is complete. Practice here uses typed input. Only compact completed outcomes are saved; conversation text is not saved by this app. OpenAI may retain data under its separate service policies.</p>
