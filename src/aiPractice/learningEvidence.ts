...
export type PracticeEvidence = {schemaVersion?:2;provenance?:PracticeSession['provenance'];inputModality?:'text'|'voice';id:string;sourceQuizId:string|null;completedAt:number;mode:PracticeSession['mode'];evaluator:'mock'|'openai'|'deterministic';epoch:number;words:EvidenceWord[]}
...
export function compactEvidence(session:PracticeSession,epoch:number,completedAt:number):PracticeEvidence{
 ...
 const evidence={...(session.provenance?{schemaVersion:2 as const,provenance:session.provenance,inputModality:session.inputModality??'text'}:{}),id:session.id,sourceQuizId:session.sourceQuizId,completedAt,mode:session.mode,evaluator:session.evaluator??'mock',epoch,words:feedback.words.map(w=>({wordId:w.wordId,direction:session.targets.find(t=>t.wordId===w.wordId)!.direction,outcome:w.outcome,retrieval:w.retrieval,semantic:w.semantic,grammar:w.grammar,suggested:feedback.suggestedReviewWordIds.includes(w.wordId)}))}
...
export function parseEvidence(value:unknown):PracticeEvidence|null{
 ...
 if(v.schemaVersion!==undefined&&(v.schemaVersion!==2||!['automatic','manual','quizFollowup'].includes(v.provenance??'')||!['text','voice'].includes(v.inputModality??'')||v.inputModality==='voice'&&(v.mode!=='conversation'||v.evaluator!=='openai')||v.provenance!=='quizFollowup'&&v.sourceQuizId!==null))return null
 if(v.schemaVersion===undefined&&(v.provenance!==undefined||v.inputModality!==undefined))return null
 ...
 return {...(v.schemaVersion===2?{schemaVersion:2 as const,provenance:v.provenance,inputModality:v.inputModality}:{}),id:v.id,sourceQuizId:v.sourceQuizId,completedAt:v.completedAt,mode:v.mode,evaluator:v.evaluator,epoch:v.epoch,words}
}
