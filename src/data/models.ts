...
export type Operation = { id: string; protocol?: 2|3|4; kind: OperationKind; at: number; notBefore?:number; undoLabel?:string; changes: Change[]; status: 'pending' | 'processing' | 'failed' | 'conflict'; message?: string; expectedRevision?: number; migrationId?: string; attempted?:boolean; retryCount?:number; lastAttemptAt?:number; nextRetryAt?:number; errorCategory?:ErrorCategory; blockedBy?:string; wirePayload?:Json }
export type SyncCache = { version: 4 | 5; quarantine?:{value:unknown;reason:string}[]; selections?:{daily?:string;review?:string}; identities: IdentityMap; migrationIdentities?: IdentityMap; base: CloudSnapshot; queue: Operation[]; backups: Operation[]; initialized: boolean; migrationChoice?: 'account' | 'imported' }
