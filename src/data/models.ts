...
export type OperationKind = 'vocabulary' | 'delete' | 'favorite' | 'learned' | 'preferences' | 'start' | 'draft' | 'submit' | 'assess' | 'migration' | 'archive'
export type Operation = { id: string; protocol?: 2; kind: OperationKind; at: number; changes: Change[]; status: 'pending' | 'conflict'; message?: string; expectedRevision?: number; migrationId?: string }
export type SyncCache = { version: 2; selections?:{daily?:string;review?:string}; identities: IdentityMap; migrationIdentities?: IdentityMap; base: CloudSnapshot; queue: Operation[]; backups: Operation[]; initialized: boolean; migrationChoice?: 'account' | 'imported' }
