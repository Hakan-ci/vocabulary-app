...
const kinds=['vocabulary','delete','bulk-delete','restore','clear-user','reset-progress','clear-all','favorite','learned','preferences','start','draft','submit','assess','migration','archive']
...
export function migrateQueue(value:SyncCache):SyncCache {
  ...
  for(const raw of value.queue) {
    ...
    if(op.wirePayload){const payload=op.wirePayload as Record<string,unknown>;if(payload.id!==op.id||payload.kind!==op.kind||payload.at!==op.at||!equal(payload.changes,op.changes)){quarantine.push({value:raw,reason:'Frozen payload does not match operation identity.'});continue}}
  ...
  return {...value,version:5,queue:compacted,...(quarantine.length?{quarantine}:{})}
}
