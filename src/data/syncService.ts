...
export class SyncService {
  ...
  cancelPending(id:string){const op=this.cache.queue.find(item=>item.id===id);if(!op||op.status!=='pending'||this.inFlightId===id)return false;this.cache.queue=this.cache.queue.filter(item=>item.id!==id);this.persist();return true}
  ...
  async flush() {
    ...
    try {
      ...
      for(const op of this.cache.queue) {
        ...
        if(op.notBefore&&op.notBefore>Date.now())continue
        ...
        this.inFlightId=op.id;const result=await this.transport.push(op);this.inFlightId=undefined
        if(this.disposed)break
    ...
    finally {this.inFlightId=undefined;this.running=false;this.syncing=false;this.emit();if(!this.error){const pending=this.cache.queue.filter(op=>op.status==='pending'),due=pending.some(op=>!op.notBefore||op.notBefore<=Date.now()),next=Math.min(...pending.map(op=>op.notBefore??Infinity));if(due)this.schedule();else if(Number.isFinite(next))this.schedule(Math.max(0,next-Date.now()))}}
  }
