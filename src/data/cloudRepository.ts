...
export function cloudRepository(owner:string):CloudTransport {return {
  async pull(){if(!supabase)throw Error('Supabase is not configured.');const {data,error}=await supabase.rpc('kelime_snapshot_v4',{}).abortSignal(AbortSignal.timeout(15000));if(error)throw error;return snapshot(data,owner)},
  async reconcile(ids){if(!supabase)throw Error('Supabase is not configured.');const {data,error}=await supabase.rpc('kelime_reconcile',{p_ids:ids}).abortSignal(AbortSignal.timeout(15000));if(error)throw error;return reconciliation(data,owner,ids)},
  async compactDrafts(operations){if(!supabase)throw Error('Supabase is not configured.');const {data,error}=await supabase.rpc('kelime_compact_drafts',{p_operations:json(operations.map(op=>({...((op.wirePayload??op) as object),accountId:owner}))),p_batch:newId()}).abortSignal(AbortSignal.timeout(15000));if(error)throw error;return reconciliation(data,owner,operations.map(op=>op.id))},
  async push(operation){if(!supabase)throw Error('Supabase is not configured.');const {data,error}=await supabase.rpc('kelime_apply_v4',{p_operation:json({...((operation.wirePayload??operation) as object),accountId:owner})}).abortSignal(AbortSignal.timeout(15000));if(error){if(error.code==='KS409')return {conflict:true,message:error.message,snapshot:await this.pull()};throw error}const result=data as unknown as {conflict:boolean;snapshot:unknown;message?:string};if(typeof result?.conflict!=='boolean')throw Object.assign(Error('Invalid operation response. Local actions are preserved.'),{code:'KS422'});return {...result,snapshot:snapshot(result.snapshot,owner)}},
}}
