...
export function Account({app,autoPronunciation,onAutoPronunciationChange}:{app:Application;autoPronunciation:boolean;onAutoPronunciationChange:(value:boolean)=>void}) {
  ...
  return <section className="test-panel account-panel" aria-label="Account">
    ...
    {!app.configured?<p>Synchronization is not configured yet. Your vocabulary and practice continue locally. Follow SUPABASE_SETUP.md to connect a project.</p>:!app.user||!app.authenticated?<>
      ...
      <div className="test-actions"><button className="secondary-button" onClick={()=>void run(()=>app.auth.signOut())} disabled={busy}>Sign out</button><button className="secondary-button" disabled={busy||app.sync?.syncing} onClick={()=>void run(()=>app.retry())}>Retry sync</button>{app.scope==='guest'&&!app.migrationOpen&&<button className="primary-button" onClick={()=>app.beginMigration()}>Set up synchronization</button>}</div>
    ...
    {app.sync&&<><p>Pending changes: {app.sync.counts.pending} · Conflicts: {app.sync.counts.conflicts}</p>{app.sync.counts.failed>0&&<p>Failed changes: {app.sync.counts.failed} — retry available</p>}{app.sync.counts.blocked>0&&<p>{app.sync.counts.blocked} changes waiting for an earlier action</p>}{!!app.sync.cache.quarantine?.length&&<p>{app.sync.cache.quarantine.length} invalid actions preserved for diagnosis</p>}{app.sync.cache.queue.filter(op=>op.status==='conflict').map(op=><div className="sync-conflict" key={op.id}><h3>{op.kind} conflict</h3><p>{op.message}</p><details><summary>Saved device action</summary><pre>{app.exportConflict(op.id)}</pre></details><button className="secondary-button" onClick={()=>app.sync?.resolve(op.id,'account')}>Continue with account data</button>{['vocabulary','favorite','learned','preferences'].includes(op.kind)&&<button className="secondary-button" onClick={()=>app.sync?.resolve(op.id,'device')}>Apply my values to current account</button>}</div>)}{app.sync.cache.backups.length>0&&<details><summary>Preserved actions ({app.sync.cache.backups.length})</summary><pre>{JSON.stringify(app.sync.cache.backups,null,2)}</pre></details>}</>}
