import {reviewEligible} from './reviewEligibility'
...
const AIPractice = lazy(() => import('./aiPractice/AIPractice').then(module => ({default:module.AIPractice})))
...
function Workspace({app}: {app: Application}) {
  ...
  const [view, setView] = useState<'all' | 'favorites' | 'test' | 'aiPractice' | 'review' | 'learned' | 'progress' | 'account'>('all')
  const [practiceQuiz,setPracticeQuiz]=useState<TestSession|null>(null)
  ...
  const queue = useMemo(() => reviewQueue(words, history, clock.now,learningStore.value.reviewRequests), [words, history, clock.now,learningStore.value.reviewRequests])
  ...
  const startReview = () => {
    ...
    const reviewSession = createReviewSession(words, current.history, clock.refresh(),Math.random,current.reviewRequests)
  ...
  const filtered = queryCatalog(words, history, favorites, filters, search, sort, clock.now, view === 'favorites' || view === 'learned' ? view : 'all',learningStore.value.reviewRequests)
  const counts = vocabularyCounts(words,history,clock.now,learningStore.value.reviewRequests)
  ...
  const changeView = (next: typeof view) => { stopPronunciation();setPracticeQuiz(null);setSelecting(false);setSelected(new Set());setView(next); clearFilters(); clock.refresh() }
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        ...
        <button className={`nav-item ${view === 'aiPractice' ? 'active' : ''}`} onClick={() => changeView('aiPractice')} aria-current={view === 'aiPractice' ? 'page' : undefined}><Icon name="book" /> AI Practice</button>
    ...
    <div className="main-shell">
      ...
      <main>
        ...
        {view === 'aiPractice' ? <AIPractice key={learningStore.value.learningEpoch} learning={learningStore.value} persistence={app.aiPracticeCommands()} history={history} catalog={words} onExit={() => changeView('all')} /> : view === 'account' ? <Account app={app} autoPronunciation={learningStore.value.autoPronunciation} onAutoPronunciationChange={setAutoPronunciation} /> : view === 'progress' ? <Progress data={dashboard(words, history, favorites, learningStore.value.activity, clock.now,learningStore.value.reviewRequests)} goal={learningStore.value.dailyGoal} onGoal={setGoal} onReview={() => changeView('review')} /> : view === 'review' ? <PracticeResume key="review" app={app} source="review" onStart={startReview}><Review requests={learningStore.value.reviewRequests} history={history} now={clock.now} catalog={words} queue={queue} session={learningStore.value.reviewSession} autoPronunciation={learningStore.value.autoPronunciation} onAutoPronunciationChange={setAutoPronunciation} onStart={startReview} draftScope={app.draftScope} onSubmit={reviewSubmit} onAssess={reviewAssess} /></PracticeResume> : view === 'test' && practiceQuiz && session?.syncId===practiceQuiz.syncId && session?.phase==='completed' ? <AIPractice persistence={app.aiPracticeCommands()} quiz={practiceQuiz} history={history} catalog={words} onExit={() => setPracticeQuiz(null)} /> : view === 'test' ? <PracticeResume key="daily" app={app} source="daily" onStart={startTest}><DailyTest requests={learningStore.value.reviewRequests} history={history} now={clock.now} catalog={words} mode={learningStore.value.preferredMode} onModeChange={setMode} autoPronunciation={learningStore.value.autoPronunciation} onAutoPronunciationChange={setAutoPronunciation} session={session} onStart={startTest} draftScope={app.draftScope} onSubmit={submit} onAssess={assess} onLearned={() => changeView('learned')} onPracticeAI={() => {if(session?.phase==='completed'){stopPronunciation();setPracticeQuiz(session)}}} /></PracticeResume> : <>
