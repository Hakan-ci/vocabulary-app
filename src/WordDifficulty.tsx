...
export function WordDifficulty({ history, now, status, needsReview }: { history: WordLearningHistory; now: number; status: LearningStatus; needsReview?:boolean }) {
  ...
  return <div className="word-difficulty">
    <div className="learning-status-row">
      ...
      {(needsReview??calculated.needsReview) ? <span className="learning-status status-needs-review">Needs Review</span> : status !== 'New' && <span className={`learning-status status-${status.toLowerCase()}`}>{status}</span>}
    </div>
