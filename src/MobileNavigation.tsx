...
export type View='all'|'favorites'|'test'|'review'|'learned'|'progress'|'account'
export function MobileNavigation({view,onChange,reviewCount}:{view:View;onChange:(view:View)=>void;reviewCount:number}) {
 ...
 return <><nav className="mobile-nav" aria-label="Mobile navigation">{([['all','Vocabulary','▤'],['test','Daily Test','✎'],['review','Review','↻'],['progress','Progress','▥']] as const).map(([value,label,symbol])=><button key={value} aria-current={view===value?'page':undefined} onClick={()=>onChange(value)}><span aria-hidden="true">{symbol}</span><span>{label}{value==='review'&&reviewCount>0?` (${reviewCount})`:''}</span></button>)}<button ref={trigger} aria-expanded={open} aria-haspopup="dialog" onClick={()=>setOpen(true)}><span aria-hidden="true">•••</span><span>More</span></button></nav><dialog ref={dialog} className="more-drawer" aria-labelledby="more-title" onCancel={close} onClose={()=>setOpen(false)}><div className="drawer-heading"><h2 id="more-title">More from Kelime</h2><button className="secondary-button" onClick={close} autoFocus>Close</button></div>{([['favorites','Favorites'],['learned','Learned'],['account','Account']] as const).map(([value,label])=><button key={value} className="secondary-button" onClick={()=>navigate(value)}>{label}</button>)}<InstallButton/></dialog></>
}
