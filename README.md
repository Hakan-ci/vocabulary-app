...
Open **AI Practice** directly, choose Automatic Practice or 1–8 words, then Use the Word, Conversation — Text, or Translation Check (typed). Daily Test remains an independent standard quiz and retains its optional AI follow-up. Mock/offline use is the default. Approved accounts can use the text pilot and separately gated Conversation — Voice. Voice uses GPT-Live 1 through supervised WebRTC and transcript review before the existing structured evaluator. Only compact outcomes and explicitly confirmed review requests persist; conversation/audio stay transient. Apply migrations through **008** before this client. The voice pilot remains disabled until server secrets, the authenticated cleanup scheduler and opt-in live checks are verified. See [architecture and deployment](AI_PRACTICE.md) and [live evaluation checklist](VOICE_EVALUATION.md).
