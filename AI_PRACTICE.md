...
### Invalid-output handling (23 September 2026)

A production `502 invalid_output` reports a rejected Responses envelope or feedback, not a database outage. The old response did not identify the rejecting check, so the exact reported production failure cannot be reconstructed from that code alone. The generation schema now bounds all text fields, restricts word IDs to selected targets and evidence/correction IDs to finalized learner turns, and requires the target count. Empty learner histories require empty evidence and correction arrays. These constraints use the [official Structured Outputs schema support](https://developers.openai.com/api/docs/guides/structured-outputs). Semantic contradictions, duplicate outcomes and unsupported corrections remain subject to the unchanged application validators.

Rejected output returns a fixed, content-free `detail` code: `output_incomplete`, `output_refused`, `output_json`, `output_text_bounds`, `output_target`, `output_evidence`, `output_contradiction`, `output_correction`, or `output_structure`. The browser shows application-owned guidance, never raw provider messages. Invalid feedback cannot complete or persist learning evidence. There is no automatic paid retry or raw-response logging.

Deploy the corrected backend with `supabase functions deploy ai-practice` and publish the updated client through the existing hosting workflow. No SQL migration is required. Deployment and paid live verification were not performed as part of this fix; the production failure is not confirmed resolved until the updated function is exercised. Voice connection troubleshooting remains separate.

Verification for this patch: Node 24.21 full unit/database/sync suite passed (26 test files); focused OpenAI Playwright suite passed (4 tests, Chromium); lint, TypeScript/production build, Deno Edge Function check and `git diff --check` passed. Browser tests required permission to start the local server outside the sandbox. No paid provider calls were made. Changed files: `supabase/functions/ai-practice/contract.ts`, `supabase/functions/ai-practice/handler.ts`, `src/aiPractice/providerError.ts` (new), `src/aiPractice/providerConnection.ts`, `src/aiPractice/practiceService.ts`, `tests/openAIPractice.test.mjs`, and `AI_PRACTICE.md`.

### Voice failure classification (23 September 2026)

Voice creation previously inherited the HTTP handler's `invalid_request` category even after validation succeeded. Thus provider, budget and sideband failures misleadingly returned HTTP 400. Creation now returns application-owned codes identifying reservation failure (503), limits (429), provider rejection, uncertain creation, invalid provider output, tracking failure or supervision failure (502). Malformed client payloads still return 400; cancellation still returns timeout. Cleanup and conservative reservations remain unchanged, with no automatic paid retries or raw provider data exposure. Redeploy only the `ai-practice` function for these diagnostic changes; no migration or frontend deployment is required. The original production voice failure remains unidentified until a new classified response is available. Focused voice/backend and text-provider regression tests, lint, Deno type checks and diff checks passed; no live voice calls or deployment performed.

### Supervision transport compatibility (23 September 2026)

A production `voice_supervision_failed` identifies failure after provider creation and tracking, while establishing trusted sideband supervision. It does not identify the precise socket error. The native `WebSocket(url, {headers})` constructor relied on a Deno-specific extension. The backend now uses pinned `npm:ws@8.18.3` with explicit Authorization headers through `sidebandSocket.ts`, avoiding that runtime dependency. The OpenAI key remains server-only; the adapter never puts it into URLs, events or logs. Redirects are disabled, handshake timeout is 10 seconds, payload size is bounded, binary events are ignored and disposal terminates the socket. Supervision, deadline enforcement, hangup and budget reservations remain mandatory. There is no model fallback or automatic paid retry.

References: [OpenAI sideband controls](https://developers.openai.com/api/docs/guides/voice-server-controls) specify the authenticated attach endpoint; [Supabase WebSockets](https://supabase.com/docs/guides/functions/websockets) documents outbound and Node-compatible WebSockets. This change addresses a concrete portability risk, but cannot establish the precise production cause without a live connection result.

Verification: all 26 Node 24 unit/database/sync test files passed; lint, Deno backend type checks and diff checks passed. `tests/sidebandHandshake.deno.ts` additionally passed using a real loopback WebSocket server and dummy key, verifying Authorization and the supervision lifecycle. Run it explicitly with `deno test --config supabase/functions/ai-practice/deno.json --allow-net=127.0.0.1 --allow-env=WS_NO_BUFFER_UTIL,WS_NO_UTF_8_VALIDATE tests/sidebandHandshake.deno.ts`. No OpenAI calls, audio, deployment or migration occurred. Redeploy `ai-practice` using `npx supabase functions deploy ai-practice`; the frontend and database need no changes. Changed files: `index.ts`, `supervisor.ts`, new `sidebandSocket.ts` and dependency `deno.lock` in `supabase/functions/ai-practice/`; `tests/voiceBackend.test.mjs`; new `tests/sidebandHandshake.deno.ts`; and this document.
