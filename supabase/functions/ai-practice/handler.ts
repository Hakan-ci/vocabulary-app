import { handleVoice, VoiceRequestError } from './voice.ts'
...
export function createHandler(deps:BackendDependencies){return async(request:Request)=>{
  ...
  }catch(error){if(!signal.aborted&&error instanceof VoiceRequestError)return reply(error.status,{error:error.code});return reply(category==='invalid_request'?400:502,{error:signal.aborted?'timeout':category,...(!signal.aborted&&category==='invalid_output'?{detail:outputFailureDetail(error)}:{})})}
  finally{
