...
export function parseRequest(value:unknown){
  ...
  const targets=list(c.targets,8).map(raw=>{
    const t=obj(raw),d=obj(t.difficulty),q=obj(t.quiz),r=obj(t.recent)
