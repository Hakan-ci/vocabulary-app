...
export async function handleVoice(raw:unknown,owner:string,deps:Dependencies,signal:AbortSignal){
 ...
 if(!deps.voiceEnabled||!deps.supervise)throw new VoiceRequestError(503,'voice_unavailable')
 ...
 try{reservation=await deps.rpc('kelime_voice_reserve',{p_owner:owner,p_attempt:input.attemptId,p_session:input.sessionId,p_fingerprint:hash})}catch{throw new VoiceRequestError(503,'voice_reservation_unavailable')}
 if(reservation!=='reserved'){
  const codes:Record<string,string>={budget:'voice_budget',rate_limit:'voice_rate_limit',busy:'voice_busy',duplicate:'voice_duplicate',unavailable:'voice_unavailable'}
  throw new VoiceRequestError(429,typeof reservation==='string'?codes[reservation]??'voice_limit':'voice_limit')
 }
 let providerId:string|null=null,sideband:Sideband|undefined
 let failure='voice_creation_uncertain'
 try{
  ...
  if(!response.ok)throw new VoiceRequestError(502,'voice_provider_rejected')
  failure='voice_invalid_output'
  ...
  try{sideband?.close()}catch{/* Scheduled cleanup retains responsibility. */}
  if(!providerId||!await hangup(deps,owner,input.attemptId).catch(()=>false))await state(deps,owner,input.attemptId,'uncertain').catch(()=>null)
  throw error instanceof VoiceRequestError?error:new VoiceRequestError(502,failure)
 }
}
