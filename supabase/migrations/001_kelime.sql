...
create table public.test_sessions (
 id uuid not null, user_id uuid not null references public.profiles(id) on delete cascade,
 ...
 state jsonb not null, updated_at timestamptz not null default now(), revision bigint not null default 0, primary key(user_id,id)
);
...
begin
 if ref ~ '^b:[0-9]+$' then
  ...
  if not exists(select 1 from public.user_vocabulary where user_id=owner_id and id=substring(ref from 3)::uuid and (historical or deleted_at is null)) and not (historical and exists(select 1 from public.account_records where user_id=owner_id and key='archived-word/'||ref)) then raise exception 'Personal vocabulary is missing, deleted, or belongs to another account'; end if;
...
create function public.kelime_deadline(s jsonb) returns timestamptz language sql immutable set search_path='' as $$
 ...
 else public.kelime_time(s->'lastKnownAt') + interval '24 hours' * case when (s->>'consecutiveKnown')::integer>=4 then 14 when (s->>'consecutiveKnown')::integer=3 then 7 when (s->>'consecutiveKnown')::integer=2 then 3 else 1 end end
$$;
...
begin
 ...
 for change in select * from jsonb_array_elements(p_operation->'changes') loop
  k:=change->>'key';
  if k !~ '^(word|archived-word|progress|favorite|deleted)/(b:[0-9]+|u:[0-9a-fA-F-]{36})$' and k !~ '^suppressed/b:[0-9]+$' and k not in ('setting/goal','setting/mode','session/daily','session/review','activity/start','activity/undated') and k !~ '^activity/[0-9]{4}-[0-9]{2}-[0-9]{2}$' then raise exception 'Invalid record key'; end if;
 ...
 for change in select * from jsonb_array_elements(p_operation->'changes') c order by case when c->>'key' like 'word/%' or c->>'key' like 'archived-word/%' then 0 else 1 end loop
  k:=change->>'key';v:=change->'after';ref:=split_part(k,'/',2);
  if k like 'archived-word/%' and (ref not like 'u:%' or v->>'id' is distinct from ref or not exists(select 1 from jsonb_array_elements(p_operation->'changes') c where c->>'key'='deleted/'||ref and c->'after'='true'::jsonb) and not exists(select 1 from public.account_records where user_id=owner_id and key='deleted/'||ref and value='true'::jsonb)) then raise exception 'Invalid archived word'; end if;
 ...
 for change in select * from jsonb_array_elements(p_operation->'changes') c where c->>'key' like 'deleted/%' loop
  ...
  if exists(select 1 from public.account_records r cross join lateral jsonb_array_elements(case when r.key='session/review' then r.value->'practice'->'questions' else r.value->'questions' end) queued_question where r.user_id=owner_id and r.key in ('session/daily','session/review') and (case when r.key='session/review' then r.value->'practice'->>'phase' else r.value->>'phase' end)<>'completed' and queued_question->>'wordId'=ref) then raise exception 'Finish the practice session before deleting this word'; end if;
