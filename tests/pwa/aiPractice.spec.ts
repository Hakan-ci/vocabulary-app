...
function completed(){let state=emptyLearningState(words,Date.now());state.session=createSession(state.history,Date.now(),()=>.3,'turkishToEnglish',[],words);while(state.session!.phase!=='completed'){state={...state,session:submitAnswer({...state.session!,draft:'wrong'})};state=assessLearningState(state,false,Date.now(),words)}return state}
...
for(const mode of ['conversation','useTheWord','voiceAnswer'])test(`${mode}: typed practice and feedback make no storage writes`,async({page})=>{
 await setup(page)
 ...
 await page.getByRole('button',{name:'Stage selected review requests'}).click();await expect(page.getByRole('status')).toContainText('1 review request staged temporarily')
 await page.getByRole('button',{name:'Stage selected review requests'}).click();await expect(page.getByRole('status')).toContainText('1 review request staged temporarily')
 expect(await page.evaluate(()=>(window as any).__practiceWrites)).toEqual([]);expect(await page.evaluate(()=>localStorage.getItem('kelime-learning-state'))).toBe(before)
