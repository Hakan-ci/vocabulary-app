...
test('clear levels preserve exactly the intended history and settings',()=>{
 ...
 const reset=resetVocabularyProgress(initial,now);assert.equal(reset.vocabulary.overrides[0],undefined);assert.equal(reset.favorites.length,0);assert.equal(reset.learning.dailyGoal,25);assert.ok(reset.learning.activity.days['2033-05-18'])
 const everything=clearVocabularyData(initial,now);assert.equal(everything.learning.dailyGoal,10);assert.equal(everything.learning.preferredMode,'englishToTurkish');assert.deepEqual(everything.learning.sessions,{});assert.deepEqual(everything.learning.activity.days,{})
})
